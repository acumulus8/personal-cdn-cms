...
builder.mutationField('createMobileApp', (t) =>
    t.prismaField({
        ...
        resolve: (query, _root, args, ctx) => {
            ...
            const { name, slug, projectId, pageId, platform, type, category, description } = args.input;
...
builder.mutationField('updateMobileApp', (t) =>
    t.prismaField({
        ...
        resolve: (query, _root, args, ctx) => {
            if (!ctx.user) throw new GraphQLError('Not authenticated', { extensions: { code: 'UNAUTHENTICATED' } });
            const { name, slug, pageId, platform, type, category, description } = args.input;
            return prisma.mobileApp.update({
                ...
                data: {
                    ...
                    ...(pageId !== undefined && pageId !== null ? { pageId } : {}),
